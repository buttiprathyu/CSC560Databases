...
Status HFPage::deleteRecord(const RID &rid)
{
    ...
    if (rid.pageNo == curPage && rid.slotNo >= 0)
    {
        ...
        if (rid.slotNo < slotCnt)
        {
           ...
           // cout<<" The offset ptr "<<offset_memadd<<" the usedptr before "<<usedPtr<<" After "<<usedPtr + mem_len<<endl;
...
Status HFPage::getRecord(RID rid, char *recPtr, int &recLen)
{
    ...
    memcpy(recPtr, &data[slot[rid.slotNo].offset - recLen], recLen * sizeof(char));
